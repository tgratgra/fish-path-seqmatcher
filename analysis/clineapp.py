	...
	* deleting the ClineApp object, which triggers cleanup (optional)
	...
	def call_cmdline (self, *clargs):
		...
		try:
			...
			stdout_hndl = open (os.path.join (workdir or '', 'stdout.txt') , 'w')
			stderr_hndl = open (os.path.join (workdir or '', 'stderr.txt') , 'w')
			self._proc = subprocess.Popen (cmdline, stdout=stdout_hndl,
				stderr=stderr_hndl, cwd=workdir, shell=True)
